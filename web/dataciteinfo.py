...
import util.datacite as datacite
import os


def dataciteinfo(request):
    wp = g.templatewp()
    ...
    try:
        ...
        def submit_xmlform(self, msg):
            print("Form submitted for showing DataCite json")
            #switch if 
        ...
        if g.loggedin:
            ...
            if True:
                ...
                if str(result[0]) == "200":
                    ...
                    # create table grid for DataCite info
                    wp.add(jp.P(text="DataCite Info ", classes="m-2 text-xl"))
                    ...
                    submit_button = jp.Input(
                        value="register/update DataCite DOI",
                        type="submit",
                        a=registerform,
                        classes=g.datacitebutton,
                    )
...
def get_datacitejson(agency, Id, additionalInfo):
    """
    get the DataCite json for a study with metadata from the Colectica API
    
    with additionalInfo=True, new information is added (compared to the dara registrations)
    """
    ...
    try:
        ...
        if True:
            ...
            if not os.path.exists(outdir):
                os.makedirs(outdir)
            jsonfile = "datacite_" + str(Id) + ".json"  # json file name
            ...
            # write DataCite json to file (use the combined_xml[Id] here; StudyURL needs to be injected - is not in DDIxml )
            datacite.write_datacitejson_fromddixml(
                os.path.join(outdir, jsonfile), combined_xml[Id], CVCollection, StudyURL, additionalInfo
            )
            print("createdatacite: " + Id + " " + str(jsonfile) + "")

            # read the created file
            f = open(os.path.join(outdir, jsonfile), "r", encoding="utf-8")
            datacitejson = f.read()
            f.close()
