...
def annotations(request):
    ...
    def submit_xmlform2(self, msg):
        ...
        for row in grid.options.rowData:
            row['anno'] = 'For Xtabs see https://search.gesis.org/variables/exploredata-' + StudyNo + '_Var' + row['variable']
    ...
    def submit_sendform2(self, msg):
        ...
        try: 
            ...
            if jsonitem['Item'] is not None:
                ...
                newxml = ed.AddAnnotation(varxml, jsonitem['Version'], './/{ddi:logicalproduct:3_3}Variable/{ddi:reusable:3_3}UserAttributePair', 'Crosstabs', 'TEST')
    ...
    if g.loggedin:
        ...
        wp.add(jp.P(text='Annotations for variables in study with agency ' + agency + ' and id ' + Id, classes='m-2'))
        
        
        #create table grid for variables of study 
        grid_options = GetGridOptions()
        grid = jp.AgGrid(a=wp, options=grid_options, style='height: 320px;width: 1100px;margin: 0.1em;' ) #style='height: 200px; width: 300px; margin: 0.25em'
        #grid.html_columns = [4]
        ...
        if True:
            ...
            if str(result[0])=='200':
                ...
                if result[1]['Item'] is not None:
                    ...
                    if True:
                        if True: # get all referenced variables 
                            ...
                            addxml=''
                            pixml=''
                            ref = ed.get_reference(studyunitxml, './/StudyUnit/PhysicalInstanceReference')
                            ...
                            if not pixml=='':
                                ...
                                addxml=''
                                ref = ed.get_reference(pixml, './/PhysicalInstance/DataRelationshipReference')
                                itm = c.get_an_item_version(ref['Agency'], ref['ID'], ref['Version'])
                                if itm is not None:
                                    if itm[1]['Item'] is not None:
                                        addxml=itm[1]['Item']
                            ...
                            if not drxml=='':        
                                ...
                                addxml=''
                                reflist = ed.get_referencelist(drxml, './/DataRelationship/LogicalRecord/VariablesInRecord/VariableUsedReference')
                                i = 1
                                for ref in reflist:
                                    ...
                                    if i<Limit:
                                        ...
                                        print('Variable found: ' + VarName + ' ' + VarLabel)                    
                                        ...
                                        """
                                        anno = ed.getCustomField(addxml, './/Variable/UserAttributePair', 'Crosstabs')
                                        
                                        
                                        row = {'study': StudyNo, 'variable': VarName, 'label': VarLabel, 'anno': anno}
                ...
                xmlform2submit_button = jp.Input(value='Create Annotations in table', type='submit', a=xmlform2, classes=g.annotationbutton)
                ...
                sendform2submit_button = jp.Input(value='Set Annotations in Colectica', type='submit', a=sendform2, classes=g.annotationbutton)
