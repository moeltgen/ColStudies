            ...
            if True:
                ...
                if str(result[0])=='200':
                    ...
                    if not g.daradoiprefix=='':
                        warndiv = jp.Div(text='DOI prefix is replaced by ' + g.daradoiprefix + ' (settings.py)', a=wp, classes='text-red-700 whitespace-pre font-mono')

                    buttonsdiv = jp.Div(text='', a=wp, classes=g.menuul, style='display: flex;')
                    #Form to show xml 
                    xmlform = jp.Form(a=buttonsdiv, classes='border m-1 p-1')
                    submit_button = jp.Input(value='show dara xml', type='submit', a=xmlform, classes=g.darabutton)
                    xmlform.on('submit', submit_xmlform)
                    
                    #Form to register
                    registerform = jp.Form(a=buttonsdiv, classes='border m-1 p-1')
                    submit_button = jp.Input(value='register/update DOI', type='submit', a=registerform, classes=g.darabutton)
                    registerform.on('submit', submit_form)
                    
                    infodiv = jp.Div(text='', a=wp)
                    registerstatus = jp.Span(text='', a=infodiv, classes='text-red-700 whitespace-pre font-mono') 
