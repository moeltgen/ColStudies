...
def login(request):
    ...
    if not g.loggedin:
        ...
        if g.status=="failedLogin":
            Message = jp.P(text='Login not successful', a=wp, classes='block uppercase tracking-wide text-red-700 text-xs font-bold mb-2')
            ...
            user_label = jp.Label(text='User Name', classes='block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2', a=loginform)
            usern = jp.Input(placeholder='User Name', a=loginform, classes='form-input')
            user_label.for_component = usern

            password_label = jp.Label(classes='block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2 mt-2', a=loginform)
            ...
            submit_button = jp.Input(value='Login', type='submit', a=loginform, classes=g.button)
            ...
            Message = jp.P(text='Login successful', a=wp, classes='block uppercase tracking-wide text-red-700 text-xs font-bold mb-2')
