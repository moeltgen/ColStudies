...
def studies(request):
    ...
    try:
        ...
        if g.loggedin:
            SeriesAgency = ''
            SeriesId = ''
            if "agency" in request.path_params: SeriesAgency = request.path_params["agency"]
            if "id" in request.path_params: SeriesId = request.path_params["id"]
            ...
            if not SeriesAgency=='' and not SeriesId=='':
                ...
                if str(result[0])=='200':
                    if result[1]['Item'] is not None:
                        ...
                        refliststudies = ed.get_referencelist(SeriesXML, './/Group/StudyUnitReference')
            ...
            if L:
                if str(L[0])=='200':
                    ...
                    wp.add(jp.P(text='Studies total: '+str(L[1]['TotalResults'])+'', classes='m-2'))
