...
def ddixml_to_study(ddixml, cvcollection, studyurl):
    ...
    study['No'] = ed.getValueFromXPath(root, './/StudyUnit/UserID[@typeOfUserID="GESIS Study Number"]')
    ...
    #AccessClass
    study['AccessClass'] = ed.getValueFromXPathWithAttribute(root, './/StudyUnit/Citation/accessRights', xml+'lang', 'de') #same for both languages 
    ...
    #Titles 
    study['TitleDE'] = ed.getValueFromXPathWithAttribute(root, './/StudyUnit/Citation/Title/String', xml+'lang', 'de')
    study['TitleEN'] = ed.getValueFromXPathWithAttribute(root, './/StudyUnit/Citation/Title/String', xml+'lang', 'en')
    study['AlternateTitleTitleDE'] = ed.getValueFromXPathWithAttribute(root, './/StudyUnit/Citation/AlternateTitle/String', xml+'lang', 'de')
    study['AlternateTitleTitleEN'] = ed.getValueFromXPathWithAttribute(root, './/StudyUnit/Citation/AlternateTitle/String', xml+'lang', 'en')
    study['SubTitleTitleDE'] = ed.getValueFromXPathWithAttribute(root, './/StudyUnit/Citation/SubTitle/String', xml+'lang', 'de')
    study['SubTitleTitleEN'] = ed.getValueFromXPathWithAttribute(root, './/StudyUnit/Citation/SubTitle/String', xml+'lang', 'en')
    
    #Creators will be filled with Name, FirstName, Affiliation
    study['Creators'] = getCreatorList(root, './/StudyUnit/Citation/Creator', 'CreatorName/String', xml+'lang', 'en', 'CreatorName', 'affiliation')
    
    #Contributors currently not present in ed index, but needed:  // todo: Roles
    study['Contributors'] = getCreatorList(root, './/StudyUnit/Citation/Contributor', 'ContributorName/String', xml+'lang', 'en', 'ContributorName', 'affiliation')
    
    #Publisher 
    study['Publisher'] = 'GESIS - Leibniz Institute for the Social Sciences' #fixed value   
    ...
    #List of collection dates, may be YYYY or YYYY-MM or YYYY-MM-DD (for StartDate, optional also for EndDate)
    study['CollDatesStart'] = getCollDatesList(root, './/StudyUnit/Coverage/TemporalCoverage/ReferenceDate', 'StartDate')
    study['CollDatesEnd'] = getCollDatesList(root, './/StudyUnit/Coverage/TemporalCoverage/ReferenceDate', 'EndDate')
    ...
    if not (study['AccessClass'] == '0' or study['AccessClass'] == 'A' or study['AccessClass'] == 'B' or study['AccessClass'] == 'C'): 
    ...
    study['AbstractDE'] = ed.getValueFromXPathWithAttribute(root, './/StudyUnit/Abstract/Content', xml+'lang', 'de')
    study['AbstractEN'] = ed.getValueFromXPathWithAttribute(root, './/StudyUnit/Abstract/Content', xml+'lang', 'en')
    
    study['PurposeDE'] = ed.getValueFromXPathWithAttribute(root, './/StudyUnit/Purpose/Content', xml+'lang', 'de')
    study['PurposeEN'] = ed.getValueFromXPathWithAttribute(root, './/StudyUnit/Purpose/Content', xml+'lang', 'en')
    
    study['FurtherRemarksDE'] = ed.getValueFromXPathCustomField(root, './/StudyUnit/UserAttributePair', 'Weitere Hinweise')
    study['FurtherRemarksEN'] = ed.getValueFromXPathCustomField(root, './/StudyUnit/UserAttributePair', 'Further Remarks')
    ...
    #More study information from List of CVs 
    study['ModeCollsEN'] = getCVTerms(root, './/Methodology/DataCollectionMethodology/TypeOfDataCollectionMethodology', False, cvcollection['TypeOfDataCollectionMethodology'])
    study['ModeCollsDE'] = getCVTerms(root, './/Methodology/DataCollectionMethodology/TypeOfDataCollectionMethodology', True, cvcollection['TypeOfDataCollectionMethodology'])
    study['SampProcsEN'] = getCVTerms(root, './/Methodology/SamplingProcedure/TypeOfSamplingProcedure', False, cvcollection['TypeOfSamplingProcedure'])
    study['SampProcsDE'] = getCVTerms(root, './/Methodology/SamplingProcedure/TypeOfSamplingProcedure', True, cvcollection['TypeOfSamplingProcedure'])
    study['TimeMethsEN'] = getCVTerms(root, './/Methodology/TimeMethod/TypeOfTimeMethod', False, cvcollection['TypeOfTimeMethod'])
    study['TimeMethsDE'] = getCVTerms(root, './/Methodology/TimeMethod/TypeOfTimeMethod', True, cvcollection['TypeOfTimeMethod'])
    study['AnalysisUnitsEN'] = getCVTerms(root, './/StudyUnit/AnalysisUnit', False, cvcollection['AnalysisUnit'])
    study['AnalysisUnitsDE'] = getCVTerms(root, './/StudyUnit/AnalysisUnit', True, cvcollection['AnalysisUnit'])
    study['KindOfDataEN'] = getCVTerms(root, './/StudyUnit/KindOfData', False, cvcollection['KindOfData'])
    study['KindOfDataDE'] = getCVTerms(root, './/StudyUnit/KindOfData', True, cvcollection['KindOfData'])
    #and freetexts
    study['ModeCollFreeEN'] = ed.getValueFromXPathWithAttribute(root, './/Methodology/DataCollectionMethodology/Description/Content', xml+'lang', 'en')
    study['ModeCollFreeDE'] = ed.getValueFromXPathWithAttribute(root, './/Methodology/DataCollectionMethodology/Description/Content', xml+'lang', 'de')
    study['SampProcFreeEN'] = ed.getValueFromXPathWithAttribute(root, './/Methodology/SamplingProcedure/Description/Content', xml+'lang', 'en')
    study['SampProcFreeDE'] = ed.getValueFromXPathWithAttribute(root, './/Methodology/SamplingProcedure/Description/Content', xml+'lang', 'de')
    study['TimeMethFreeEN'] = ed.getValueFromXPathWithAttribute(root, './/Methodology/TimeMethod/Description/Content', xml+'lang', 'en')
    study['TimeMethFreeDE'] = ed.getValueFromXPathWithAttribute(root, './/Methodology/TimeMethod/Description/Content', xml+'lang', 'de')
    study['AnalysisUnitFreeEN'] = ed.getValueFromXPathWithAttribute(root, './/StudyUnit/AnalysisUnitsCovered/String', xml+'lang', 'en')
    study['AnalysisUnitFreeDE'] = ed.getValueFromXPathWithAttribute(root, './/StudyUnit/AnalysisUnitsCovered/String', xml+'lang', 'de')
    study['KindOfDataFreeEN'] = '' #not supported 
    study['KindOfDataFreeDE'] = '' #not supported 
    
    #ISO nation codes 
    study['GeoIDs'] = ed.getListFromXPath(root, './/StudyUnit/Coverage/SpatialCoverage/CountryCode')    
    ...
    study['NumberOfUnits'] = ed.getValueFromXPath(root, './/PhysicalInstance/GrossFileStructure/CaseQuantity')
    study['AnalysisSystemsEN'] = ed.getValueFromXPathWithAttribute(root, './/PhysicalInstance/Citation/format', xml+'lang', 'en')
    study['AnalysisSystemsDE'] = ed.getValueFromXPathWithAttribute(root, './/PhysicalInstance/Citation/format', xml+'lang', 'de')
    
    study['NumberOfVariables'] = str(getCountOfElements(root, './/Variable'))
    
    #Topics 
    study['TopicsEN'] = ed.getListFromXPath(root, './/StudyUnit/Coverage/TopicalCoverage/Keyword') 
    study['TopicsDE'] = []                               #todo get German Text from English Text (no CV is used here) 
    ...
    #Groups
    study['GroupIDs'] = ed.getListFromXPath(root, './/Group/UserID[@typeOfUserID="GESIS Group Number"]')           #DBK GroupNumbers
    study['GroupTitlesEN'] = ed.getListFromXPathWithAttribute(root, './/Group/Citation/Title', 'String', xml+'lang', 'en')
    study['GroupTitlesDE'] = ed.getListFromXPathWithAttribute(root, './/Group/Citation/Title', 'String', xml+'lang', 'de')
    study['GroupDescriptionsEN'] = ed.getListFromXPathWithAttribute(root, './/Group/Abstract', 'Content', xml+'lang', 'en')
    study['GroupDescriptionsDE'] = ed.getListFromXPathWithAttribute(root, './/Group/Abstract', 'Content', xml+'lang', 'de')
    study['GroupLinksEN'] = ed.getListFromXPathCustomField(root, './/Group/UserAttributePair', 'URL-en')
    study['GroupLinksDE'] = ed.getListFromXPathCustomField(root, './/Group/UserAttributePair', 'URL-de')
    ...
    study['GroupLogo'] = ed.getListFromXPathCustomField(root, './/Group/UserAttributePair', 'Logo') #so use custom field instead 
    ...
    study['StudyLanguageIds'] = LanguageList          #ISO language codes 2-letter: in those languages there are questions (associated to variables) available  
    study['StudyLanguageNamesEN'] = getLanguageNames(study['StudyLanguageIds'])
    study['StudyLanguageNamesDE'] = []                                                  #todo: is there a library for German country names?
    study['StudyLanguageCountryIds'] = LanguageList #is ISO language codes, extended by - and a country code for specification (e.g.: de-AT, fr-BE, it-CH)
...
def getCreatorList(node, xpath, subpath, attributename, attributevalue, affiliationpath, affiliationatt):
...
def getAccessClassText(accessClass, useGerman):
    ...
    if accessClass == '0':
        ...
        if useGerman: text = 'Daten und Dokumente sind für jedermann freigegeben.'
    elif accessClass == 'A':
        text = 'Data and documents are released for academic research and teaching.'
        if useGerman: text = 'Daten und Dokumente sind für die akademische Forschung und Lehre freigegeben.'
    elif accessClass == 'B':
        ...
        if useGerman: text = 'Daten und Dokumente sind für die akademische Forschung und Lehre freigegeben, wenn die Ergebnisse nicht veröffentlicht werden. \
        ...
        if useGerman: text = 'Daten und Dokumente sind für die akademische Forschung und Lehre nur nach schriftlicher Genehmigung des Datengebers zugänglich. \
...
def getOtherMatList(node, xpath):
    ...
    for uid in userids_all:
        ...
        if omtype == 'PublishedDocumentationFile' or omtype == 'PublishedDataFile' or omtype== 'FreeDataFile':
            titlede = ''
            titleen = ''
...
def get_cvinfo(ddixml, itemxpath):
    ...
    #Get properties 
    ref['Agency'] = ed.getAttributeValueFromXPath(root, itemxpath, 'controlledVocabularyAgencyName')
    ref['ID'] = ed.getAttributeValueFromXPath(root, itemxpath, 'controlledVocabularyID')
    ref['Version'] = ed.getAttributeValueFromXPath(root, itemxpath, 'controlledVocabularyVersionID')    
